...
public class UsuarioRegistrar extends JFrame implements ActionListener {
    ...
    public void actionPerformed(ActionEvent evento) {
        if (evento.getSource() == SaveNewUser) {
            ...
            System.out.println(getUsuario.CrearUsuario(datosUsuario, password));
            if (getUsuario.CrearUsuario(datosUsuario, password) == 1) {
                JOptionPane.showMessageDialog(null, "Ok");
            } else {
                JOptionPane.showMessageDialog(null, "User or/and password are diferent");
            }
        }
